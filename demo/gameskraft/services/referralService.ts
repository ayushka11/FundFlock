...
export default class ReferralService {
    ...
    static async getUserReferees(restClient: any, userId: number, vendorId: string, refereeFilter: number, pagination: Pagination) {
        try {
            ...
            const userRefereeResp: any = await ReferralClient.getUserReferees(restClient, userId, refereeFilter, pagination);
    ...
    static async getReferralDetails(restClient: any, userId: number, vendorId: string) {
        try {
            ...
            let referralDetails: any = await ReferralClient.getReferralDetails(restClient, userId);
    ...
    static async getUserReferralStats(restClient: any, userId: number) {
        try {
            ...
            const userReferralStats: any = await ReferralClient.getUserReferralStats(restClient, userId);
    ...
    static async getUserByReferCode(restClient: any, referCode: string) {
        try {
            logger.info(`[ReferralService] [getUserByReferCode] referCode :: ${referCode}`);
            const userByReferCodeResp: any = await ReferralClient.getUserByReferCode(restClient, referCode);
            logger.info(`[ReferralService] [getUserByReferCode] referCode :: ${referCode} userByReferCodeResp :: ${JSON.stringify(userByReferCodeResp)}`);
            return userByReferCodeResp;
        } catch (e) {
            logger.error(e,`[ReferralService] [getUserByReferCode] Error :: `);
            throw e;
        }
    }

    static async addUserReferral(restClient: any, userId: number, referrerUserId: number) {
        try {
            logger.info(`[ReferralService] [addUserReferral] userId :: ${userId} referrerUserId :: ${referrerUserId}`);
            const addUserReferralResp: any = await ReferralClient.addReferral(restClient, userId, referrerUserId);
    ...
    static async createUserReferCode(restClient: any, userId: number) {
        try {
            logger.info(`[ReferralService] [createUserReferCode] userId :: ${userId}`);
            const createUserReferCodeResp: any = await ReferralClient.createUserReferCode(restClient, userId);
            logger.info(`[ReferralService] [createUserReferCode] userId :: ${userId} createUserReferCodeResp :: ${JSON.stringify(createUserReferCodeResp)}`);
            return createUserReferCodeResp;
        } catch (e) {
            logger.error(e,`[ReferralService] [createUserReferCode] Error :: `);
            throw e;
        }
    }

    static async validateReferCode(restClient: any, referCode: string) {
        try {
            logger.info(`[ReferralService] [validateReferCode] referCode :: ${referCode}`);
            const validateReferCodeResp: any = await ReferralClient.validateReferCode(restClient, referCode);
            logger.info(`[ReferralService] [validateReferCode] referCode :: ${referCode} validateReferCodeResp :: ${JSON.stringify(validateReferCodeResp)}`);
            return validateReferCodeResp;
        } catch (e) {
            logger.error(e,`[ReferralService] [validateReferCode] Error :: `);
            throw e;
        }
    }

    static async getUserReferralSharePayload(restClient: any, userId: number) {
        try {
            logger.info(`[ReferralService] [getUserReferralSharePayload] userId :: ${userId}`);
            const userReferralSharePayload: any = await ReferralClient.getUserReferralSharePayload(restClient, userId);
            logger.info(`ReferralService] [getUserReferralSharePayload] userId :: ${userId} userReferralSharePayload :: ${JSON.stringify(userReferralSharePayload)}`);
            return userReferralSharePayload;
        } catch (e) {
            logger.error(e,`[ReferralService] [getUserReferralSharePayload] Error :: `);
            throw e;
        }
    }
